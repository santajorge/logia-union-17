...
export default function ColumnaSurPage() {
  ...
  if (cargando) return <p style={{ padding: '2rem', color: '#1a1a2e' }}>Pasando lista en la Columna Sur...</p>
    ...
    <div style={{ maxWidth: '1000px', paddingBottom: '3rem', color: '#1a1a2e' }}>
      ...
      <div style={{ marginBottom: '2rem', borderBottom: '1px solid #e8e6e0', paddingBottom: '1.5rem' }}>
        <h1 style={{ fontSize: '28px', fontWeight: '600', color: '#1a1a2e', margin: '0 0 8px', display: 'flex', alignItems: 'center', gap: '10px' }}>
          <Users color="#CDA434" size={28} /> Columna Sur
        </h1>
        <p style={{ fontSize: '14px', color: '#666', margin: 0 }}>Gestión de Compañeros y Plan de Instrucción.</p>
      </div>
            ...
            <div key={obrero.id} style={{ backgroundColor: '#fff', border: '1px solid #e8e6e0', borderRadius: '12px', overflow: 'hidden' }}>
                <div style={{ padding: '1.5rem', borderBottom: '1px solid #f0efe9' }}>
                  ...
                  <p style={{ fontSize: '12px', color: '#888', margin: 0, display: 'flex', alignItems: 'center', gap: '4px' }}>
                    <Clock size={12} /> Aumento: {new Date(fechaAntiguedad).toLocaleDateString('es-AR')}
                  </p>
                </div>
